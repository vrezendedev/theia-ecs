...
public sealed class RelationsMetaTests
{
    ...
    [Fact]
    public void RegisterRelation_WithType_ReturnsIndexOfValidTagRelationType()
    {
        int id = RelationsMeta.RegisterRelation(typeof(ViaTypeTagRelation));

        RelationType relationType = RelationsMeta.GetRelationType(id);

        Assert.Equal(typeof(ViaTypeTagRelation), relationType.Get());
        Assert.Equal(RelationsMeta.IsTag<ViaTypeTagRelation>(), relationType._isTag);
        Assert.NotNull(relationType);
        Assert.IsType<RelationType<ViaTypeTagRelation>>(relationType);

        Relation relation = relationType.CreateRelation();
        Assert.IsType<Relation>(relation);

        Relation firstRelationEnqueued = new Relation();
        relationType.PoolRelation(firstRelationEnqueued);

        Assert.Equal(firstRelationEnqueued, relationType.CreateRelation());

        RelationLink relationLink = relationType.CreateRelationLink();
        Assert.IsType<RelationLink>(relationLink);

        RelationLink firstRelationLinkEnqueued = new RelationLink();
        relationType.PoolRelationLink(firstRelationLinkEnqueued);

        Assert.Equal(firstRelationLinkEnqueued, relationType.CreateRelationLink());

        int idViaGeneric = RelationMeta<ViaTypeTagRelation>.s_id;

        Assert.Equal(id, idViaGeneric);
    }

    [Fact]
    public void RegisterRelation_WithType_ReturnsIndexOfValidEvaluatedRelationType()
    {
        int id = RelationsMeta.RegisterRelation(typeof(ViaTypeEvaluatedRelation));

        RelationType relationType = RelationsMeta.GetRelationType(id);

        Assert.Equal(typeof(ViaTypeEvaluatedRelation), relationType.Get());
        Assert.Equal(RelationsMeta.IsTag<ViaTypeEvaluatedRelation>(), relationType._isTag);
        Assert.NotNull(relationType);
        Assert.IsType<RelationType<ViaTypeEvaluatedRelation>>(relationType);

        Relation relation = relationType.CreateRelation();
        Assert.IsType<EvaluatedRelation<ViaTypeEvaluatedRelation>>(relation);

        EvaluatedRelation<ViaTypeEvaluatedRelation> firstRelationEnqueued =
            new EvaluatedRelation<ViaTypeEvaluatedRelation>();
        ...
        int idViaGeneric = RelationMeta<ViaTypeEvaluatedRelation>.s_id;
